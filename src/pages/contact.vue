  ...
  <div class="min-h-screen bg-background">
    ...
    <section class="relative py-10">
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-br from-primary/5 to-background"></div>
      ...
      <div class="container relative mx-auto px-4">
        <div class="max-w-3xl mx-auto text-center mb-16">
          ...
          <p class="text-xl text-muted-foreground">
            Notre équipe d'experts est prête à répondre à toutes vos questions et à vous aider à réaliser vos projets.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-16">
          ...
          <div
            ...
            <p class="text-muted-foreground mb-4">Appelez-nous du lundi au vendredi</p>
          ...
          <div
            ...
            <p class="font-medium">349 Rue de la Cavalade<br />34530 Montpellier, France</p>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
          ...
          <div class="bg-card border rounded-xl shadow-sm overflow-hidden">
            <div class="p-2 bg-gradient-to-r from-primary to-primary-foreground"></div>
            <div class="p-8">
              ...
              <Form v-slot="{ errors }" :validation-schema="schema" @submit="onSubmit">
                <div class="space-y-5">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <Label for="name" class="text-sm font-medium mb-2">Nom</Label>
                      <Field v-slot="{ field }" name="name">
                        <Input v-bind="field" id="name" placeholder="Votre nom complet" class="mt-1 w-full" />
                      </Field>
                    ...
                    <div>
                      ...
                      <Field v-slot="{ field }" name="email">
                        <Input v-bind="field" id="email" type="email" placeholder="votreemail@exemple.com"
                          class="mt-1 w-full" />
                      </Field>
                  ...
                  <div>
                    <Label for="subject" class="text-sm font-medium mb-2" >Sujet</Label>
                    ...
                    <p v-if="errors.subject" class="text-xs text-destructive mt-1">{{ errors.subject }}</p>
                  </div>

                  <div>
                    <Label for="message" class="text-sm font-medium mb-2">Message</Label>
                    <Field v-slot="{ field }" name="message">
                      <Textarea v-bind="field" id="message" placeholder="Décrivez votre demande en détail..." rows="5"
                        class="mt-1 w-full resize-none" />
                    </Field>
                    <p v-if="errors.message" class="text-xs text-destructive mt-1">{{ errors.message }}</p>
                  </div>
                  ...
                  <div class="space-y-3 pt-2">
                    <div class="flex items-start space-x-3">
                      ...
                      <label for="privacy" class="text-sm leading-tight cursor-pointer">
                        J'accepte la <NuxtLink to="/legal/terms-of-use" class="text-primary hover:underline">politique
                          de
                          confidentialité</NuxtLink>
                      </label>
                    </div>
                    <p v-if="errors.privacy" class="text-xs text-destructive mt-1">{{ errors.privacy }}</p>

                    <div class="flex items-start space-x-3">
                      <Checkbox id="newsletter" v-model="newsletter" />
                      <label for="newsletter" class="text-sm leading-tight cursor-pointer">
                        Je souhaite recevoir la newsletter et les actualités de VizHome (optionnel)
                      </label>
                    </div>
                  </div>

                  <Button type="submit" class="w-full bg-primary font-medium py-2.5 mt-2">
                    Envoyer le message
                  </Button>
                </div>
              </Form>
              ...
              <div class="space-y-4">
                <Accordion type="single" collapsible>
                  <AccordionItem value="item-1" class="border-none">
                    <AccordionTrigger class="py-2 text-base">Quel est le délai de réponse ?</AccordionTrigger>
                    <AccordionContent class="text-sm text-muted-foreground">
                      Nous répondons généralement dans les 24h ouvrées, et sous 4h pour les clients Pro et Entreprise.
                    </AccordionContent>
                  </AccordionItem>
                  <AccordionItem value="item-2" class="border-none">
                    <AccordionTrigger class="py-2 text-base">Comment programmer une démo ?</AccordionTrigger>
                    <AccordionContent class="text-sm text-muted-foreground">
                      Sélectionnez "Demande commerciale" comme sujet dans le formulaire pour demander une démo
                      personnalisée.
                    </AccordionContent>
                  </AccordionItem>
                </Accordion>
                <div class="pt-2">
                  <NuxtLink to="/faq" class="text-primary hover:underline text-sm font-medium inline-flex items-center">
...
import { MailIcon, PhoneIcon, MapPinIcon, ArrowRightIcon } from 'lucide-vue-next'
