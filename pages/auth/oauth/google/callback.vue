...
onMounted(async () => {
  ...
  const redirect = sessionStorage.getItem('google_oauth_redirect') || '/render'
  sessionStorage.removeItem('google_oauth_state')
  sessionStorage.removeItem('google_oauth_redirect')

  // Google a renvoyé une erreur (user a refusé, scope manquant…)
  if (oauthError) {
    state.value = 'error'
    errorMessage.value =
      oauthError === 'access_denied'
        ? "Tu as refusé l'autorisation Google."
  ...
  try {
    ...
    const user = await auth.loginGoogle({ code, redirectUri })
    await fetchMe()
